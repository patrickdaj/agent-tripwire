# transcript-audit Specification

## Purpose

Expose the detector entry point that audits tool-call transcripts against the
agent's stated task into validated `TranscriptAudit` results, with cross-field
consistency between the verdict and the per-resource scope judgments, and a
