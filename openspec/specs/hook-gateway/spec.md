...
Provide the `agent-tripwire-gate` console script — the stable stdin/stdout seam
that every harness adapter speaks. In neutral mode it evaluates a `GateRequest`
and emits a `GateDecision`; malformed input fails closed; and a Claude Code
adapter mode translates harness events into gate requests and back, carrying no
