# sensitive-output-detection Specification

## Purpose

Expose the detector entry point that flags sensitive data in model outputs into
validated `SensitiveDataFlag` results, with cross-field consistency between the
verdict and its findings, and a deterministic offline mock provider.
