...
Expose a single `evaluate(request)` entry point that routes each agent surface
to its detector, escalates from an offline mock tier to a configured model when
needed, maps detector verdicts to intervention decisions, and always fails
closed — returning a validated `GateDecision` (never an unhandled exception)
within a bounded deadline, with an operator escape hatch.
